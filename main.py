...
def get_best_direct_video_url(video_page_url: str) -> Dict[str, Any]:
    ...
    a direct media URL (HLS or file; prefer MP4 formats when possible).
    Returns:
      - direct_url
      - ext
      - id
...
def stream_video_to_r2(media_url: str, r2_key: str) -> None:
    ...
    Note: if media_url is an HLS manifest, you are actually saving the
    manifest text, not a flattened MP4.
    """
...
async def api_rumble_r2(req: RumbleRequestWithLimit):
    ...
    Paging + streaming:
      ...
      5) Return R2 public URLs + metadata + direct_url.
    ...
    try:
        ...
        for idx, page_url in enumerate(page_urls, start=start + 1):
            entry: Dict[str, Any] = {
                ...
                "r2_key": None,
                "r2_url": None,
                "direct_url": None,            # NEW: for ffmpeg in Colab
                "duration": None,
                "title": None,
            }
            try:
                ...
                entry["r2_key"] = r2_key
                entry["r2_url"] = r2_url
                entry["direct_url"] = media_url   # expose real media URL
